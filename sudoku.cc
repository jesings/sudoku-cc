...
void Update(array<array<bitset<9>, 9>, 9> &possibilities, int i, int j,
            char value) {
  ...
  for (int k = 0; k < 9; k++) {
    ...
    possibilities[box_start_row + k / 3][box_start_col + k % 3].reset(value);
  }
  possibilities[i][j] = bitset<9>();
}

// Finds if there is only one square in the box, column, or row (whose tiles are
// given by putting the values 0-8 into locator) that can have a given value, if
// so put that value in that tile, and update possibilities, board, and
// unfilled.
void FindForcing(array<array<bitset<9>, 9>, 9> &possibilities,
...
// Finds if there are n squares in the box, column, or row (whose tiles are
// given by putting the values 0-8 into locator) that must hold the same n
// values. (e.g. if there are 2 squares which could either be a 2 or a 4) then
// ensure that the possibilities table marks those values as impossible in any
// other tile in the box, column, or row.
...
void solveSudoku(vector<vector<char>> &board) {
  ...
  while (unfilled > 0) {
    ...
    if (prev_unfilled == unfilled) {
      ...
      for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
          if (board[i][j] == '.' && possibilities[i][j].count() < min) {
...
int main() {
  ...
  for (int i = 0; i < 9; i++) {
    ...
    for (char c : s) {
      if ((c < '1' || c > '9') && c != '.') {
        cout << "The valid characters for the board are 1 through 9 and . to "
                "specify unknown"
             << endl;
        return -1;
      }
